"""
================================================================================
  THESIS: Quantitative Analysis and Optimization of Blockchain-Induced
          Latency in Real-Time Embedded Flight Control Systems
  Author : Nabil / merajpi
  Hardware: Raspberry Pi 5 (Ubuntu) + Pixhawk 2.4.8
  Logic  : Multithreaded Latency Measurement (End-to-End T_total)
================================================================================
"""

import time
import curses
import csv
import os
import sys
import threading
import queue
import copy
import statistics
from datetime import datetime
from collections import deque

from pymavlink import mavutil
from web3 import Web3
from web3.middleware import ExtraDataToPOAMiddleware

# ============================================================
#  CONFIGURATION
# ============================================================
DEVICE           = '/dev/ttyAMA0'
BAUD             = 921600
DISPLAY_INTERVAL = 0.1
TX_INTERVAL      = 2.0  # Seconds between blockchain submissions

RPC_URL          = 'http://127.0.0.1:8545'
CHAIN_ID         = 1337
MY_ADDRESS       = "0x55fa363e65c1cd9172F8D1E34FFD4A35A52f3998"

LOG_DIR          = "/home/merajpi/Nabil/logs"
WINDOW_SIZE      = 50

# ============================================================
#  GLOBAL SHARED STATE
# ============================================================
w3 = Web3(Web3.HTTPProvider(RPC_URL))
w3.middleware_onion.inject(ExtraDataToPOAMiddleware, layer=0)

# Queue to send data from Telemetry Loop to Blockchain Thread
tx_queue = queue.Queue()

# Thread-safe dictionary for the UI to read status updates
blockchain_stats = {
    "latency_ms": 0.0,
    "status": "INITIALIZING...",
    "tx_count": 0,
    "current_block": 0
}

os.makedirs(LOG_DIR, exist_ok=True)
timestamp_str = datetime.now().strftime("%H%M-%d%m%Y")
log_filename  = os.path.join(LOG_DIR, f"latency_analysis_{timestamp_str}.csv")

CSV_HEADER = [
    "Timestamp", "Telemetry_Gap_ms", "Telemetry_Mean_Gap_ms", "Telemetry_Max_Gap_ms",
    "Telemetry_Std_Gap_ms", "Blockchain_T_total_ms", "TX_Count", "Block_Number",
    "Blockchain_Status", "Altitude_m", "Speed_mps", "Heading_deg",
    "Latitude", "Longitude", "Voltage_V", "Current_A", "Satellites"
]

# ============================================================
#  HELPERS & BACKGROUND WORKER
# ============================================================
def append_csv(row: list):
    with open(log_filename, mode='a', newline='') as f:
        csv.writer(f).writerow(row)

def blockchain_worker():
    """
    Background thread: Measures T_total.
    This thread blocks on w3.eth.wait_for_transaction_receipt, 
    allowing the main flight loop to remain responsive.
    """
    try:
        nonce = w3.eth.get_transaction_count(MY_ADDRESS, 'pending')
    except:
        nonce = 0

    while True:
        # Wait for data from the main telemetry loop (Producer)
        telemetry_snapshot, gap_stats = tx_queue.get()
        
        # START TIMER (Initial Hashing/Submission phase)
        t_start = time.perf_counter()
        
        try:
            payload = (
                f"ALT:{telemetry_snapshot['altitude']:.2f},"
                f"SPD:{telemetry_snapshot['speed']:.2f},"
                f"LAT:{telemetry_snapshot['latitude']:.6f},"
                f"LON:{telemetry_snapshot['longitude']:.6f}"
            )

            tx = {
                'from': MY_ADDRESS, 'to': MY_ADDRESS, 'value': 0,
                'gas': 120_000, 'gasPrice': max(w3.eth.gas_price, w3.to_wei(1, 'gwei')),
                'nonce': nonce, 'data': w3.to_hex(text=payload), 'chainId': CHAIN_ID
            }

            # SUBMIT TRANSACTION (T_h)
            tx_hash = w3.eth.send_transaction(tx)
            
            # WAIT FOR MINING (T_c + T_p)
            # This is the 'Blocking' call that now happens in the background
            receipt = w3.eth.wait_for_transaction_receipt(tx_hash)
            
            # CALCULATE END-TO-END (T_total)
            t_total = (time.perf_counter() - t_start) * 1000.0
            
            # Update shared state for the Curses UI
            blockchain_stats.update({
                "latency_ms": t_total, "status": "OK",
                "tx_count": blockchain_stats["tx_count"] + 1,
                "current_block": receipt['blockNumber']
            })

            # Log to CSV including the confirmed block number
            append_csv([
                datetime.now().isoformat(), gap_stats['gap'], gap_stats['mean'],
                gap_stats['max'], gap_stats['std'], f"{t_total:.3f}",
                blockchain_stats["tx_count"], receipt['blockNumber'], "OK",
                f"{telemetry_snapshot['altitude']:.3f}", f"{telemetry_snapshot['speed']:.3f}",
                telemetry_snapshot['heading'], f"{telemetry_snapshot['latitude']:.7f}",
                f"{telemetry_snapshot['longitude']:.7f}", f"{telemetry_snapshot['voltage']:.3f}",
                f"{telemetry_snapshot['current']:.3f}", telemetry_snapshot['satellites']
            ])
            nonce += 1

        except Exception as e:
            blockchain_stats["status"] = f"ERR: {str(e)[:30]}"
            blockchain_stats["latency_ms"] = -1.0
            # If transaction fails, sync nonce with the blockchain
            nonce = w3.eth.get_transaction_count(MY_ADDRESS, 'pending')
        
        tx_queue.task_done()

def safe_addstr(stdscr, y, x, text, attr=0):
    h, w = stdscr.getmaxyx()
    if y < h and x < w:
        try: 
            stdscr.addstr(y, x, str(text)[:w-x-1], attr)
        except: 
            pass

# ============================================================
#  MAIN TELEMETRY LOOP (PRODUCER)
# ============================================================
def main(stdscr):
    # Curses Setup
    curses.curs_set(0)
    stdscr.nodelay(True)
    curses.start_color()
    
    # FIX: Initialize default colors to allow -1 (transparent) background
    curses.use_default_colors() 
    
    curses.init_pair(1, curses.COLOR_GREEN, -1)
    curses.init_pair(2, curses.COLOR_RED, -1)
    curses.init_pair(3, curses.COLOR_CYAN, -1)

    # Initialize CSV Header
    with open(log_filename, mode='w', newline='') as f:
        csv.writer(f).writerow(CSV_HEADER)

    # Connect MAVLink (Serial)
    try:
        connection = mavutil.mavlink_connection(DEVICE, baud=BAUD)
        connection.wait_heartbeat(timeout=5)
        connection.mav.request_data_stream_send(
            connection.target_system, connection.target_component,
            mavutil.mavlink.MAV_DATA_STREAM_ALL, 20, 1
        )
    except Exception as e:
        print(f"MAVLink Connection Error: {e}")
        return

    # Start the Blockchain Background Worker
    worker_thread = threading.Thread(target=blockchain_worker, daemon=True)
    worker_thread.start()

    # Telemetry State
    telemetry_gaps = deque(maxlen=WINDOW_SIZE)
    last_msg_time = last_display_time = time.perf_counter()
    last_tx_time = time.perf_counter() - TX_INTERVAL
    
    telemetry = {
        'latitude': 0.0, 'longitude': 0.0, 'altitude': 0.0, 'pos_z': 0.0,
        'voltage': 0.0, 'current': 0.0, 'speed': 0.0, 'heading': 0, 'satellites': 0
    }
    height_offset = None

    while True:
        # Check for user input
        key = stdscr.getch()
        if key == ord('q'): break
        if key == ord('r'): height_offset = None
        
        # 1. Non-blocking read from Serial Port
        msg = connection.recv_match(blocking=False)
        if msg:
            now = time.perf_counter()
            gap = (now - last_msg_time) * 1000.0
            telemetry_gaps.append(gap)
            last_msg_time = now

            # Parse MAVLink Data
            m = msg.get_type()
            if m == 'GLOBAL_POSITION_INT':
                telemetry['latitude'], telemetry['longitude'] = msg.lat/1e7, msg.lon/1e7
            elif m == 'VFR_HUD':
                telemetry['speed'], telemetry['heading'] = msg.groundspeed, msg.heading
            elif m == 'SYS_STATUS':
                telemetry['voltage'] = msg.voltage_battery / 1000.0
                telemetry['current'] = msg.current_battery / 100.0
            elif m == 'LOCAL_POSITION_NED':
                if height_offset is None: height_offset = -msg.z
                telemetry['altitude'] = -msg.z - height_offset
            elif m == 'GPS_RAW_INT':
                telemetry['satellites'] = msg.satellites_visible

            # 2. Blockchain Trigger (Handoff to Worker)
            if now - last_tx_time >= TX_INTERVAL:
                last_tx_time = now
                gaps_list = list(telemetry_gaps)
                
                # Bundle current statistics
                stats_snapshot = {
                    'gap': f"{gap:.2f}", 
                    'mean': f"{statistics.mean(gaps_list):.2f}",
                    'max': f"{max(gaps_list):.2f}",
                    'std': f"{statistics.stdev(gaps_list) if len(gaps_list)>1 else 0:.2f}"
                }
                
                # Send deepcopy of telemetry to thread to avoid race conditions
                tx_queue.put((copy.deepcopy(telemetry), stats_snapshot))

        # 3. UI Dashboard Refresh
        if time.perf_counter() - last_display_time >= DISPLAY_INTERVAL:
            stdscr.erase()
            safe_addstr(stdscr, 0, 0, "=== BLOCKCHAIN FLIGHT LATENCY ANALYSIS (T_TOTAL) ===", curses.A_REVERSE)
            
            # Telemetry link health
            current_gap = telemetry_gaps[-1] if telemetry_gaps else 0
            safe_addstr(stdscr, 2, 0, f"MAVLink Serial Gap: {current_gap:8.2f} ms", curses.color_pair(3))
            
            # Shared Blockchain Stats
            safe_addstr(stdscr, 4, 0, "BLOCKCHAIN PERFORMANCE (End-to-End)", curses.A_BOLD)
            bc_col = curses.color_pair(1) if blockchain_stats["status"] == "OK" else curses.color_pair(2)
            
            safe_addstr(stdscr, 5, 2, f"T_total Latency  : {blockchain_stats['latency_ms']:8.2f} ms", bc_col | curses.A_BOLD)
            safe_addstr(stdscr, 6, 2, f"Confirmed Block  : #{blockchain_stats['current_block']}")
            safe_addstr(stdscr, 7, 2, f"TX Count         : {blockchain_stats['tx_count']}")
            safe_addstr(stdscr, 8, 2, f"Node Status      : {blockchain_stats['status']}", bc_col)
            
            # Flight Data
            safe_addstr(stdscr, 10, 0, "CURRENT FLIGHT STATE", curses.A_BOLD)
            safe_addstr(stdscr, 11, 2, f"Alt: {telemetry['altitude']:.2f}m | Spd: {telemetry['speed']:.2f}m/s | Hdg: {telemetry['heading']}")
            safe_addstr(stdscr, 12, 2, f"GPS: {telemetry['latitude']:.6f}, {telemetry['longitude']:.6f} (Sats: {telemetry['satellites']})")
            
            safe_addstr(stdscr, 14, 0, "[q] Quit  [r] Reset Alt Baseline", curses.color_pair(3))
            stdscr.refresh()
            last_display_time = time.perf_counter()

if __name__ == "__main__":
    try:
        curses.wrapper(main)
    except KeyboardInterrupt:
        print("\nSession stopped by user.")
    except Exception as e:
        print(f"\n[FATAL] {e}")